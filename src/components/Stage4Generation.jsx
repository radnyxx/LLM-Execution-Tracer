...
// Helper for the delay logic we added earlier
const delay = (ms) => new Promise(res => setTimeout(res, ms));

export default function Stage4Generation({ schema, temperature, onNeuralUpdate, groq }) {
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        if (content) {
          ...
          await delay(70); // The slow-down logic
        }
      }
    } catch (e) {
      if (e.name !== 'AbortError') setDisplayed(p => p + "\n[SYSTEM_ERROR_FAILED_INFERENCE]");
    } finally {
      setLoading(false);
  ...
  return (
    <div style={terminalContainer}>
      ...
      <div style={headerMetrics}>
        <div style={metricItem}>STREAM: <span style={{ color: loading ? '#00ff9d' : '#475569' }}>{loading ? 'ACTIVE' : 'IDLE'}</span></div>
        <div style={metricItem}>TOK/SEC: <span style={{ color: '#00d4ff' }}>14.2</span></div>
        <div style={metricItem}>INF_MODE: <span style={{ color: '#f8fafc' }}>FP16_QUANT</span></div>
...
// STYLES - Hardcoded to ensure no external dependencies break it
...
const runBtn = { background: '#00ff9d', color: '#020617', border: 'none', borderRadius: 2, padding: '2px 8px', fontSize: 8, fontWeight: 900, cursor: 'pointer' };
