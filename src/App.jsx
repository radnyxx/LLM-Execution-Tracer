...
import { DEFAULT_SCHEMA, shannonEntropy, bpeSplit, tokenHex } from './schema';
import './index.css';

const TOUR_STEPS = [
  { stage: 'tokenize', title: 'STEP 1 // TOKENIZATION', body: 'Your prompt is split into tokens — sub-word pieces the model can process. Each token gets a unique ID and an attention weight that signals its importance.', highlight: 0 },
  { stage: 'embed', title: 'STEP 2 // EMBEDDING SPACE', body: 'Each token is mapped to a high-dimensional vector (embedding). Similar words cluster nearby. The dashed lines show cosine similarity — how semantically related tokens are.', highlight: 1 },
  { stage: 'sample', title: 'STEP 3 // ATTENTION + SOFTMAX', body: 'The model computes attention — which tokens matter for generating the next word. Softmax converts raw scores (logits) into probabilities. Try dragging the temperature slider!', highlight: 2 },
  { stage: 'generate', title: 'STEP 4 // GENERATION', body: 'Token by token, the model samples from the probability distribution and streams the output. Each word\'s brightness shows its confidence. The KV Cache tab shows memory being built up.', highlight: 3 },
];
...
export default function App() {
  ...
  const [tourStep, setTourStep] = useState(-1); // -1 = no tour
    ...
    <div style={containerStyle} className="grid-bg">
        ...
        <div style={welcomeOverlayStyle} onClick={() => setShowWelcome(false)}>
          <div style={welcomeBoxStyle} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: 10, color: '#64748b', lineHeight: 1.6, marginBottom: 20 }}>
              Visualize how a large language model processes your prompt — from tokenization to token generation. Type a prompt, adjust temperature, and watch every stage of the LLM pipeline in real time.
            </p>
      ...
      <header style={headerStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span style={{ fontSize: 9, color: neuralState.isProcessing ? '#00ff9d' : '#334155', fontWeight: 800 }} className={neuralState.isProcessing ? 'pulse' : ''}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <main style={gridStyle}>
          ...
          <div style={{
            ...
            <Stage1Tokenization tokens={schema.tokens} highlighted={highlightedStage === 0} />
          </div>
          ...
          <div style={{
            ...
            <Stage2Vectors tokens={schema.tokens} vectors={schema.vectors} highlighted={highlightedStage === 1} />
          </div>
